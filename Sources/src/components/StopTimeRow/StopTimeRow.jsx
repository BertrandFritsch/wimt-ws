...
const StopTimeRow = React.createClass({
  ...
  render() {
    ...
    return (
      <div className={[ 'stop-time-row', this.props.tripState && this.props.tripState.state && 'stop-time-row-real-time' ].join(' ')}
        ...
        <span className="stop-time-row-state">{realTimeStateDisplay(this.props.tripState && this.props.tripState.state)}</span>
      </div>
    );
  }
});
